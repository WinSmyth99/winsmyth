...
const HOUSE_STYLE =
  'A single iconic emblem for a game symbol: ONE centered object only, not a scene, not a machine, ' +
  'not a sign. Vice synthwave style: bold clean silhouette readable at small size, neon rim lighting ' +
  'in hot pink and violet, deep dark purple background, subtle retro sunset glow, high contrast. ' +
  'ABSOLUTELY NO text, NO letters, NO words, NO typography, NO logos, NO signage, NO watermark, NO border.';
...
export default async (req: Request) => {
  ...
  try {
    ...
    a.attempts += 1;
    const prompt = `${HOUSE_STYLE} The object: ${names[next]}. Mood: ${String(spec.tagline ?? '').replace(/["']/g, '')}.`;
